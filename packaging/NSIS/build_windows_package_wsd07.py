...
class LocalWindowsPackageBuilder:
    ...
    def _collect_binary_files(self) -> List[tuple]:
        ...
        # Add CuraEngine executable (prefer Windows version)
        curaengine_paths = [
            self.cura_root / "CuraEngine.exe",  # Windows version (preferred)
            self.cura_root / "CuraEngine"       # Unix version (fallback)
        ]
        ...
        # Add common library locations for Windows
        lib_paths.extend([
        ...
        # Add paths from PATH environment variable that might contain conan libs
        for path_dir in path_env.split(';'):
            if path_dir and ('conan' in path_dir.lower() or 'lib' in path_dir.lower()):
                lib_paths.append(path_dir)
    ...
    def _generate_spec_content(self, datas: List[tuple], binaries: List[tuple], hiddenimports: List[str], collect_all_modules: List[str]) -> str:
        ...
        icon_path = self.cura_root / "packaging" / "icons" / "Cura.ico"

        spec_content = f'''# -*- mode: python ; coding: utf-8 -*-
# PyInstaller spec file for local Windows Cura build
